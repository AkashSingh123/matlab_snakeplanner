function adj_nodes = get_ranadj_nodes_norm(A,target_point_buffer,vx,vy)
r=0;
points = A;
post=1;
state_a=0;
...
for check=1:size(points,1)
    if(round(points(check,1),2)==round(target_point_buffer(end,1),2)&& round(points(check,2),2)==round(target_point_buffer(end,2),2))
        req=check;
       
    end
end
 adj_nodes=adj_nodes_buffer{req};
 adj_nodes=get_sorted_adjnodes_norm(adj_nodes,target_point_buffer);
 
for i=1:size(adj_nodes,1)
    if(round(adj_nodes(i,1:2),2)==round(target_point_buffer(end-1,1:2),2))
        rem=i;
    end
end
...
for j=1:size(adj_nodes,1)
    if(post==1)
    if(round(adj_nodes(j,1:2),2)==round(target_point_buffer(end-4,1:2),2))
        r=j;
        post=0;
    end
    end
