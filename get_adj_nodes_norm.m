function list_adj_nodes = get_adj_nodes_norm(target_point_buffer,A,vx,vy)

list_adj_nodes=[];
for i=1:39
    a=pdist2(target_point_buffer(end,1:2),A(i,1:2));
    a=round(a);
...
for i=1:size(list_adj_nodes,1)
    if(round(list_adj_nodes(i,1:2))==round(target_point_buffer(end-1,1:2)))
        rem=i;
    end
end
